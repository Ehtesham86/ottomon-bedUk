...
const ProductCardMatress = ({ item, selectedGrid,itemOffset, pageType, CallingFrom }) => {
  ...
  console.log(selectedGrid, 'CallingFrom______')
  ...
  useEffect(() => {
    ...
    const fetchMattresses = async () => {
      try {
        const response = await axios.get("https://ottomonbackend-l-f8a1.vercel.app/mattresses");
        setLoading(false);
    ...
    Loading ? (
     ...
     ( mattresses?.map((item, index) =>
        selectedGrid === 0 ? (
          <div
          key={item._id}
          className={`flex flex-col md:flex-row p-4 gap-4 border border-primary ${index >= 0 && index < 9 ? "border-b-0" : ""}`}
        >
          <div className="overflow-hidden w-full md:w-[500px] rounded-t-xl relative">
            <span
              ref={icon1Ref}
            ...
            <img
              src={item.imageUrl}
              alt="Product"
              className="object-contain h-[200px] sm:h-[250px] md:h-[300px] w-full rounded-t-xl duration-1000 hover:scale-125"
              ref={imageRef}
            />
          </div>
          <div className="flex flex-col md:flex-row items-center justify-between w-full">
            <div className="text-center pt-3">
              <a
                ...
                className="bg-primary cursor-pointer p-2 text-white rounded-md"
              >
                <span>Customize your bed</span>
              </a>
            </div>
            <div className="px-4 py-3 w-full md:w-auto">
          ...
          className={`${
            selectedGrid === 2
              ? "md:min-w-[300px] md:max-w-[500px] md:min-w-[400px] lg:min-w-[500px] lg:max-w-[500px]"
              : "w-full"
          } bg-white shadow-md rounded-xl hover:shadow-xl overflow-hidden`}
        >
          <div
            className={`relative w-full ${
              selectedGrid === 2
                ? "md:h-[300px] md:sm:h-[400px] lg:h-[500px]"
